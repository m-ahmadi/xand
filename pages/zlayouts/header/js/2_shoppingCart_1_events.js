$('#shopping-cart').on('click', function(e) {
	...
	var parsed = SITE.pages.header.shoppingCart.parsed,
		update = SITE.pages.header.shoppingCart.update;
	
	if (id === 'sCart_button') {
		
		SITE.pages.header.minMax(id, 'sCart_open', 'sCart_min', 'sCart_max');
	}

	if (target.id === 'empty_sCart_butt') {
		SITE.pages.header.shoppingCart.clear();
	}

	if (target.hasClass('remove_sCart_item')) {
		function removeItemFromArr(target) {
			
			var pName = $(e.target).parent().parent().prev().prev().prev().children().first().html();
			var arr = parsed();
			if (arr) {
		...
		var arr = removeItemFromArr(target.target);
